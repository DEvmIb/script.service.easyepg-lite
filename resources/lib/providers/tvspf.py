...
def epg_main_converter(item, data, channels, settings, ch_id=None, genres={}):
    ...
    for row in table:
        ...
        g["title"] = columns[2].find("strong").get_text()
        ...
        if regional.get(ch_id):
            for s in selected_regionals:
                d = dict()
                d["b_id"]  = s + "|" + columns[2].find("span", recursive=False).find("a", class_="js-track-link")["href"].split("/")[-1].replace(".html", "")
                d["c_id"]  = s
                for i in g.keys():
                    d[i] = g[i]
            ...
            g["b_id"]  = columns[2].find("span", recursive=False).find("a", class_="js-track-link")["href"].split("/")[-1].replace(".html", "")
            g["c_id"]  = ch_id
...
def epg_advanced_converter(item, data, cache, settings):
    ...
    if series:
        ...
        if "Staffel " in se_info[0]:
            s_num = se_info[0].replace("Staffel ", "").split("/")[0].split("+")[0].split(";")[0].split(",")[0].split("-")[0].split(chr(8211))[0].split(chr(8212))[0]
        if len(se_info) == 1 and "Folge " in se_info[0]:
            ...
            e_num_fix = se_info[1].replace("Folge ", "").split("/")[0].split("+")[0].split(";")[0].split(",")[0].split("-")[0].split(chr(8211))[0].split(chr(8212))[0]
